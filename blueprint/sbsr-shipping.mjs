/**
 * sbsr-shipping.mjs — DETERMINISTIC ongkir + invoice for Sentuh Rasa. No LLM touches money.
 *   quoteOngkir(lat,lng,cart) → { available, ongkir, eta, courier }   (Biteship; sim fallback)
...
export function subtotalOf(cart) { return (cart || []).reduce((s, i) => s + i.price * i.qty, 0); }
function haversineKm(a1, o1, a2, o2) { const R = 6371, dLa = (a2 - a1) * Math.PI / 180, dLo = (o2 - o1) * Math.PI / 180, x = Math.sin(dLa / 2) ** 2 + Math.cos(a1 * Math.PI / 180) * Math.cos(a2 * Math.PI / 180) * Math.sin(dLo / 2) ** 2; return 2 * R * Math.asin(Math.sqrt(x)); }

export async function quoteOngkir(lat, lng, cart) {
  const distKm = haversineKm(ORIGIN.lat, ORIGIN.lng, lat, lng);
  if (SIM || !BITESHIP_KEY) { // offline: deterministic estimate so the flow is testable
  ...
  try {
    ...
    const res = await fetch("https://api.biteship.com/v1/rates/couriers", {
      ...
      body: JSON.stringify({ origin_latitude: ORIGIN.lat, origin_longitude: ORIGIN.lng, destination_latitude: lat, destination_longitude: lng, couriers: "gojek,grab", items: items.length ? items : [{ name: "order", value: 50000, quantity: 1, weight: 500 }] }),
      signal: ctrl.signal,
    });
    clearTimeout(t);
  ...
  } catch { return { available: false, distKm: +distKm.toFixed(1) }; }
}
...
export function invoiceText(order) {
