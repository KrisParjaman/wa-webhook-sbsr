...
const buildSbsrDeliveryMethodPromptText = () => 'Mau Delivery atau Pickup?';
const buildSbsrCartSummary = () => '(empty)';
const semanticAddressMatch = () => Promise.resolve(null);
const getSbsrDeterministicMissingStateMessage = () => '';
const sendSbsrLocationPromptMessage = () => {};
const sendPinConfirmPrompt = () => {};
const maybeHandleAddressPinDistanceGate = () => Promise.resolve(false);
...
const parseRupiah = () => 0;
const sniffCartAckFromAiReply = () => false;
const sniffInvoiceFromAiReply = () => false;
const enrichInvoiceWithMaps = (f,r) => r;
const maybeAutoQuote = () => Promise.resolve(false);
const maybeFireAdminEscalation = () => Promise.resolve(false);
const sendSbsrAddonOffer = () => {};
const sendSbsrDeliveryMethodButtons = () => {};
const sendSbsrUseCasePrompt = () => {};
const sendWelcomeWithCatalog = () => {};
const sbsrRouterStateLabel = () => 'initial';
const sbsrLlmClassifierEnabled = true;
...
function runQuoteFor(from, draft, courierPref) {
  return new Promise((resolve) => {
    console.error('[runQuoteFor] draft.destination='+JSON.stringify(draft?.destination)+' items='+(draft?.items?.length||0));
    if (!draft || !draft.destination) {
      console.error('[runQuoteFor] NO DESTINATION — cannot quote');
      return resolve(null);
    }
    if (!draft.destination.lat && !draft.destination.lng) {
      console.error('[runQuoteFor] NO COORDS in destination');
      return resolve(null);
    }
    const frozen = (draft.items || []).some(it => it.form === 'frozen');
    const payload = JSON.stringify({
      phone: from, items: draft.items,
      destination: { ...draft.destination }, frozen,
    ...
    child.on('close', () => {
      try { const parsed = mg.parseScriptJSON(stdout); resolve(parsed && parsed.ok ? parsed : null); }
      catch { resolve(null); }
    });
...
module.exports = { init, tryHandleAddressAndQuote, runQuoteFor };
