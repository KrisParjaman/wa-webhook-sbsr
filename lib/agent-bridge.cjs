...
async function processTurn(phone, userText, msg, messageId, contactName) {
  ...
  // 7) If delivery mode: name+address set but no location → inject location request button
  if (o.fulfillment === 'delivery' && o.name && o.address && !o.pin && !o.escalate) {
    const _reply = (r.reply || '').toLowerCase();
    if (/lokasi|share loc|kirim lokasi|share your location|share lokasi/i.test(_reply) && typeof _sendLocationRequest === 'function') {
