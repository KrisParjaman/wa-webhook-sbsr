...
let _notifyAdminWithButtons = null;
let _log = null;
...
// Content-based dedup for button clicks — prevents WhatsApp retry ghost messages
const _lastButtonClick = {};
...
function init(opts) {
  ...
  _notifyAdminWithButtons = opts.notifyAdminWithButtons || null;
  _log = opts.log || (() => {});
  ...
  if (q.courierInfo && q.courierInfo.needsFrozenChoice && q.frozenOptions && q.frozenOptions.length >= 2) {
    ...
    if (typeof _sendButtons === 'function') {
      await _sendButtons(phone, 'Pilih kurir ya Kak 🤍', [
        { type: 'reply', reply: { id: 'courier_1', title: ('🐧 ' + _opts[0].courier + ' ' + _formatRupiah(_opts[0].ongkir)).slice(0, 20) } },
        { type: 'reply', reply: { id: 'courier_2', title: ('🛵 ' + _opts[1].courier + ' ' + _formatRupiah(_opts[1].ongkir)).slice(0, 20) } },
      ]);
    }
...
async function _handlePaymentProof(phone, state, order) {
  ...
  const _adminBody = '🧾 Bukti bayar dari ' + norm(phone) + ' (' + (order.name || '?') + ').'
    + '\n\nNama: ' + (order.name || '?')
    + '\nTotal: ' + (order.grandTotal ? _formatRupiah(order.grandTotal) : '?')
    + '\n\nTap Approve/Reject atau balas:\nAPPROVE ' + norm(phone) + '\nREJECT ' + norm(phone);
  if (typeof _notifyAdminWithButtons === 'function') {
    await _notifyAdminWithButtons(_adminBody, norm(phone), '🧾 Bukti bayar (balas APPROVE/REJECT ' + norm(phone) + ')', 'payment-proof');
  } else if (typeof _notifyAdmin === 'function') {
    await _notifyAdmin(_adminBody, 'payment-proof');
  }
...
async function processTurn(phone, userText, msg, messageId, contactName) {
  ...
  // 3b) "reset" / "mulai ulang" → clear all state, trigger greeting + catalog
  if (/^(reset|mulai\s*ulang|start\s*over|ulang\s*dari\s*awal)$/i.test(text || '')) {
    // Reset to empty — runAgent will auto-prepend system prompt
    state.messages = [];  // runAgent checks length===0 → adds SYSTEM_PROMPT
    order.cart = [];
    delete order.fulfillment; delete order.name; delete order.address;
    delete order.pin; delete order.ongkir; delete order.ongkir_courier;
    delete order._catalogSent; delete order.flow; delete order.courierPreference;
    // Don't return — let it fall through to the agent for natural greeting
  }

  // 4) Deterministic fulfillment button reply — match ID OR title
  //    With content-based dedup to prevent WhatsApp retry ghost messages
  //    Customer clicks "🛵 Dikirim" or "🏪 Ambil sendiri" from interactive buttons
  var _isDeliveryBtn = /^(delivery|dikirim|🛵|kirim|antar)$/i.test(text || '');
  var _isPickupBtn = /^(pickup|ambil sendiri|🏪|pick\s*up|mampir)$/i.test(text || '');
  if ((_isDeliveryBtn || _isPickupBtn) && (order.cart || []).length > 0) {
    // Content dedup: ignore same button click within 30s (WhatsApp retry ghost)
    var _btnKey = norm(phone) + '_btn_' + (text || '').trim();
    var _now = Date.now();
    if (_lastButtonClick[_btnKey] && (_now - _lastButtonClick[_btnKey] < 30000)) {
      _log('agent', 'dup button content — skip: ' + _btnKey);
      return true;
    }
    _lastButtonClick[_btnKey] = _now;
    // If already set → guard against repeat clicks
    ...
    // First time: set fulfillment deterministically
    var _method = _isPickupBtn ? 'pickup' : 'delivery';
    order.fulfillment = _method;
    // Clear stale location/ongkir from previous sessions to prevent premature escalation
    if (_method === 'delivery' && order.pin) {
      delete order.pin; delete order.ongkir; delete order.ongkir_courier; delete order.ongkir_eta;
      delete order._frozenOptions; order.flow = '';
    }
    state.messages.push({ role: 'user', content: text });
    state.messages.push({ role: 'assistant', content: '[sistem: fulfillment = ' + _method + ' — deterministik, tanpa LLM]' });
    var _nextPrompt = _method === 'delivery'
  ...
  // o.finalize is reset to false inside _sendInvoiceAndQris before saveState,
  // so duplicate sends can't happen across turns.
  if (o.finalize) {
