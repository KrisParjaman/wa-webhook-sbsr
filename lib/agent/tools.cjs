...
register('add_to_cart', 'Tambah item ke keranjang. Panggil setelah customer sebut produk + qty.',
  ...
  (args, ctx) => {
    ...
    const catalog = (ctx.catalog || ctx._catalogSnapshot || require('../catalog-manager.cjs').loadStaticCatalog() || {});
    ...
    for (const cat of categories) {
      ...
      for (const v of cat.variants || []) {
        if (v.aliases && v.aliases.some(a => a.toLowerCase().includes(variant.toLowerCase()) || variant.toLowerCase().includes(a.toLowerCase()))) {
          product = v; break;
        }
      }
      if (product) break;
