...
function formatMenuText() {
  ...
  const hasLive = Object.keys(map).length > 0;
  if (!hasLive) {
    // Fallback to static JSON
    const p = loadStaticCatalog();
    if (!p) return 'Menu sedang tidak tersedia. Ketik *menu* lagi nanti ya Kak 🤍';
    const out = ['📋 *Menu Sentuh Rasa*', ''];
    for (const c of p.categories || []) {
      out.push('*' + c.name + '*');
      for (const v of c.variants || []) {
        const ps = Object.entries(v.prices || {}).map(([q, p]) => q + 'pcs Rp' + Number(p).toLocaleString('id-ID')).join(' / ');
        out.push('• ' + v.name + ' — ' + ps);
      }
      out.push('');
    }
  ...
  // Live catalog from PostgreSQL + Meta API
  // Check if prices are loaded (non-zero)
  let hasPrices=false;
  for (const rid in prices) { if (prices[rid]>0) { hasPrices=true; break; } }
  if (!hasPrices) {
    // Fallback to static JSON
    const p=loadStaticCatalog();
    if (!p) return 'Menu sedang tidak tersedia. Ketik *menu* lagi nanti ya Kak 🤍';
    ...
    for (const c of p.categories||[]) {
      out.push('*'+c.name+'*');
      for (const v of c.variants||[]) {
        const ps=Object.entries(v.prices||{}).map(([q,pr])=>q+'pcs Rp'+Number(pr).toLocaleString('id-ID')).join(' / ');
        out.push('• '+v.name+' — '+ps);
      }
      out.push('');
    }
  ...
  const sizeSort = { '3': 1, '6': 2, '12': 3, 'FRZ': 4 };
  const famNames = { 'RA': 'Ayam Sayur', 'RR': 'Ragout Creamy', 'RM': 'Smoked Beef Mayo', 'RAM': 'Ayam Mercon Chili Oil 🔥', 'RAP': 'Ayam Sayur Pedas', 'MIX': 'Mix Risol (pilih varian)' };

  const out = ['📋 *Menu Sentuh Rasa*', ''];

  out.push('*Risoles Goreng (Makan Langsung)*');
  for (const fKey of famOrder) {
    const items = [];
    for (const rid in map) {
      if (!Object.prototype.hasOwnProperty.call(map, rid)) continue;
      const m = rid.match(RI_RE);
      if (!m || m[1] !== fKey) continue;
      if (m[2] === 'FRZ') continue; // skip frozen in goreng section
      items.push({ size: m[2], price: prices[rid] || 0 });
    }
    items.sort((a, b) => (sizeSort[a.size] || 99) - (sizeSort[b.size] || 99));
    const parts = items.map(i => i.size + 'pcs Rp' + Number(i.price).toLocaleString('id-ID'));
    if (parts.length) out.push('• ' + (famNames[fKey] || fKey) + ' — ' + parts.join(' / '));
  }

  out.push('');
  out.push('*Risoles Frozen (Stok Rumah — 6pcs/pack)*');
  for (const fKey of famOrder) {
    for (const rid in map) {
      if (!Object.prototype.hasOwnProperty.call(map, rid)) continue;
      const m = rid.match(RI_RE);
      if (!m || m[1] !== fKey || m[2] !== 'FRZ') continue;
      const p = prices[rid] || 0;
      out.push('• ' + (famNames[fKey] || fKey).replace(' 🔥', '').replace(' (pilih varian)', '') + ' Frozen 6pcs — Rp' + Number(p).toLocaleString('id-ID'));
    }
  }
  ...
  out.push('');
  out.push('Ketik langsung produk yang Kakak mau ya. Contoh: *ayam sayur 6pcs goreng* 🤍');
  return out.join('\n');
}
