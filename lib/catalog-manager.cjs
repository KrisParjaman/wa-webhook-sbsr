...
function formatMenuText() {
  ...
  out.push('');
  out.push('*Risoles Frozen (Stok Rumah — 12pcs/pack)*');
  for (const fKey of famOrder) {
    for (const rid in map) {
      if (!Object.prototype.hasOwnProperty.call(map, rid)) continue;
      const m = rid.match(RI_RE);
      ...
      out.push('• ' + (famNames[fKey] || fKey).replace(' 🔥', '').replace(' (pilih varian)', '') + ' Frozen — Rp' + Number(p).toLocaleString('id-ID'));
    }
  }
  ...
  out.push('');
  out.push('_Harga update otomatis. Ketik langsung produk yang Kakak mau ya 🤍_');
  return out.join('\n');
}
