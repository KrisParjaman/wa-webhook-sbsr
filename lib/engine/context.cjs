...
function _srvLoadDraft(from) {
  try {
    ...
    if (fs.existsSync(p)) {
      ...
      // Auto-expire sessions older than 24h (skip terminal states)
      const LIVE_STATES = new Set(['awaiting_proof','pending_finance','approved','booked','delivered']);
      if (d.updated_at && !LIVE_STATES.has(d.state)) {
...
function _srvSaveDraft(from, draft) {
  ...
  const payload = { ...draft, updated_at: ts };
  // Sync file write (primary, fast)
  try {
    const dir = _services.SBSR_DRAFTS_DIR;
    const safe = String(from).replace(/[^0-9]/g, '');
    ...
    console.error('[ctx] saveDraft file failed:', e.message);
  }
  // Async Postgres write (fire-and-forget, B2 single source of truth)
  const pool = _services && _services.pgPool;
  if (pool) {
    const safe = String(from).replace(/[^0-9]/g, '');
    pool.query(
      `INSERT INTO wa_drafts (tenant, phone, draft, updated_at)
       VALUES ('sbsr', $1, $2, $3)
       ON CONFLICT (tenant, phone)
       DO UPDATE SET draft = EXCLUDED.draft, updated_at = EXCLUDED.updated_at`,
      [safe, payload, ts]
    ).catch(function(e) { console.error('[ctx] saveDraft pg failed:', e.message); });
  }
}
