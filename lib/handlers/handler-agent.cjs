...
const FORM_F = /\b(?:frozen|forzen|froozen|frozon|mentah|stok|beku|simpan|stock|freezer)\b/i;
...
let _catalogMediaId=null;
async function _sendCatalogImage(ctx) {
  try {
    if (!_catalogMediaId) {
      const wa=require('../wa-sender.cjs');
      const res=await wa.uploadMedia('/docker/wa-webhook-sbsr/static/catalog-image.jpeg','image/jpeg');
      ctx.log('det','catalog-upload: '+JSON.stringify(res));
      _catalogMediaId=res?.id||null;
    }
    if (_catalogMediaId) {
      await require('../wa-sender.cjs').sendImage(ctx.from,_catalogMediaId);
      ctx.log('det','catalog-image-sent');
    } else {
      ctx.log('det','catalog-no-media-id');
    }
  } catch(e) { ctx.log('det','catalog-err: '+e.message); }
}
...
async function handler(ctx) {
  ...
  if (RESET_RE.test(t) && t.length < 15) {
    ...
    ctx.saveDraft();
    _sendCatalogImage(ctx).catch(()=>{});
    ctx.replyText=`Ijin mengirimkan Menu/Pricelist ya ka😊🙏🏻\n\nRisoles Sentuh Rasa dibuat untuk kakak yang mencari cemilan dengan kualitas rasa konsisten, isian melimpah, dan bahan serta racikan yg tepat✨\n\nKarena itu banyak pelanggan kami yang akhirnya repeat order untuk stok di rumah, suguhan tamu, maupun acara kantor.\n\nBatch produksi hari ini masih tersedia ya ka, namun beberapa varian sering sold out lebih cepat🙏\n\nJika ingin mencicipi, Kakak bisa pesan sekarang untuk mengamankan stoknya, boleh diinfo mau Frozen atau goreng ka? 🥰`; ctx.handled=true; return;
  }
  // 1b. Remove item from cart ("ga jadi X", "hapus X")
  const REMOVE_RE = /\b(?:ga\s*jadi|gak\s*jadi|nggak\s*jadi|hapus|hilangkan|remove|cancel|batal(?:in)?)\s+(.+)/i;
  const rmMatch=t.match(REMOVE_RE);
  if (rmMatch && cart.length>0) {
    const target=rmMatch[1].toLowerCase();
    const idx=cart.findIndex(it=>target.includes(it.name.toLowerCase())||it.name.toLowerCase().includes(target)||(it.variant&&target.includes(it.variant)));
    if (idx>=0) {
      const removed=cart[idx];
      const newCart=cart.filter((_,i)=>i!==idx);
      const total=newCart.reduce((s,it)=>s+Number(it.unit_price||0),0);
      ctx.updateDraft({items:newCart,subtotal:total});
      ctx.saveDraft();
      ctx.replyText=`${removed.name} dihapus dari keranjang ya Kak 🤍\n\nSisa ${newCart.length} item. Total: Rp${Number(total).toLocaleString('id-ID')}. Mau tambah yang lain?`;
    } else {
      ctx.replyText=`Mintu gak nemu item "${rmMatch[1]}" di keranjang Kak. Yang ada: ${cart.map(it=>it.name).join(', ')}.`;
    }
    ctx.handled=true; ctx.log('det','remove'); return;
  }

  // 2. Status check / cart inquiry
  if (/\b(?:status|cek\s*pesanan|pesanan\s*saya|orderan|keranjang|isi\s*keranjang|belanjaan|pesananku|pesanan\s*aku)\b/i.test(t) && cart.length>0) {
  ...
  // 3. FAQ (from original SBSR_FAQ_INTENTS — only when NOT in active flow)
  if (t.length < 80 && state!=='awaiting_delivery_method' && state!=='awaiting_name' && state!=='awaiting_address') {
    // Compaint/refund → escalate to admin
    if (/\b(?:komplain|complain|keluhan|belum\s+sampai|ga\s+sampai|rasa\s+biasa|basi|produk\s+rusak|refund|retur|kembali(?:kan|in)\s+dana)\b/i.test(t)) {
      ctx.replyText='Mohon maaf ya Kak atas kendalanya 🤍\n\nBoleh kirim detail keluhannya beserta foto/video pendukung kalau ada, nanti Mintu bantu teruskan ke admin untuk ditindaklanjuti.';
      try { const fPhone=(process.env.SBSR_FINANCE_PHONES||'6285741844938').split(',')[0].trim(); require('../wa-sender.cjs').sendText(fPhone,'🚨 *KOMPLAIN* dari +'+ctx.from+': '+t.slice(0,200)); } catch(_) {}
      ctx.handled=true; ctx.log('det','faq-escalate'); return;
    }
    // FAQ patterns
    const faqs=[
      [/\bhalal\b|\bsertifikasi\s+halal\b/i,'Untuk sertifikasi halal, Sentuh Rasa saat ini sedang dalam proses ya Kak 🤍'],
      [/\b(?:tahan|awet|expired?|kadaluarsa|freezer|chiller|simpan\s+berapa|shelf\s*life)\b/i,'Frozen tahan 1-2 bulan di freezer, 1-2 hari di chiller. Suhu ruang 2-3 jam. Kalau sudah digoreng langsung santap ya 🤍'],
      [/\b(?:cara\s+goreng|gorengnya|masaknya|menggoreng)\b/i,'Cara goreng frozen: 1. Keluarkan dari freezer (tidak perlu thaw). 2. Minyak panas. 3. Api sedang sampai golden brown. 4. Tiriskan. Kalau dari chiller bisa langsung goreng 🤍'],
      [/\b(?:pickup|pick\s*up|ambil\s*sendiri|mampir|datang\s+langsung|ambil\s+di\s+toko)\b/i,'Bisa pickup di Jl Nusa Indah Raya blok O no 10, Cipinang Muara, Kec Jatinegara. Maps: https://share.google/ykWkdLTDJgG2UVfOQ. CP: +62 811 1321 166 🤍'],
      [/\b(?:reseller|agen|jualan\s+lagi|titip\s+jual)\b/i,'Reseller: 4 pack Rp47rb/pack, 6 pack Rp46rb/pack, 10 pack Rp45rb/pack. Info lebih lanjut chat admin ya 🤍'],
      [/\b(?:minimum|minimal|min)\s*(?:order|pembelian|beli|pesan)|order\s+minimal/i,'Minimum order Rp50.000 ya Kak 🤍'],
      [/\b(?:pengiriman|dikirim|kirimnya|delivery|diantar|antar)\b/i,'Pengiriman dari Cipinang, Jakarta Timur. Gojek/Grab/Paxel. Ongkir dihitung setelah alamat + pin maps dikirim 🤍'],
      [/\b(?:pembayaran|bayar|transfer|qris|rekening|metode\s+bayar)\b/i,'Pembayaran via QRIS atau transfer. Nanti setelah cart & pengiriman fix, Mintu kirim detailnya 🤍'],
      [/\b(?:air\s*fryer|airfryer)\b/i,'Bisa digoreng pakai air fryer juga ya Kak. Sesuaikan waktu & suhu sampai golden brown 🤍'],
      [/\b(?:cara\s+simpan|penyimpanan|simpan\s+frozen)\b/i,'Frozen: suhu ruang 2-3 jam, chiller 1-2 hari, freezer 1-2 bulan 🤍'],
      [/\b(?:gift|hampers|hadiah|parcel|kado)\b/i,'Untuk gift/hampers cocok pakai thermal bag premium, greeting card, mika bag, chili sauce. Mintu bantu rekomendasikan ya Kak 🤍'],
      [/\b(?:meeting|acara|event|gathering|kantor|rapat)\b/i,'Untuk meeting/acara cocok: risoles siap makan, chili sauce, mika bag, tambah minuman. Kirim estimasi pax & waktunya ya Kak 🤍'],
      [/\b(?:lokasi|alamat)\s*toko|toko\s*(?:di\s*mana|dimana)|berlokasi/i,'Sentuh Rasa di Jl Nusa Indah Raya blok O no 10, Cipinang Muara, Kec Jatinegara. Maps: https://share.google/ykWkdLTDJgG2UVfOQ 🤍'],
    ];
    for (const [re,reply] of faqs) {
      if (re.test(t)) { ctx.replyText=reply; ctx.handled=true; ctx.log('det','faq'); return; }
    }
  }
  ...
  if (PRODUCT_RE.test(t) && QTY_RE.test(t)) {
    ...
    for (const tr of triples) {
      ...
      // Frozen: minimum 6pcs (1 pack = 12pcs). Block <6pcs.
      ctx.log('det',`frozen-check qty=${tr.qty} form=${tr.form} qty<6=${tr.qty<6}`);
      if (tr.form==='frozen' && tr.qty>=1 && tr.qty<6) {
        ctx.replyText=`Maaf Kak, frozen minimal *6pcs* per pack (Rp55.000/pack).\n\nMau frozen 6pcs? Atau ganti ke *goreng* (siap makan) bisa 3pcs/6pcs/12pcs? 🤍`;
        ctx.handled=true; ctx.log('det','frozen-min-qty'); return;
      }
    ...
    ctx.saveDraft();
    ctx.log('det',`cart=${existing.length} total=${total} triples=${triples.length} firstQty=${triples[0]?.qty||'?'} firstForm=${triples[0]?.form||'?'}`);
  ...
  // 5. Product without qty
  if (PRODUCT_RE.test(t) && !QTY_RE.test(t)) {
    const p=(t.match(PRODUCT_RE)||[])[1]||'';
    const pendingQty=ctx.draft.pending_qty;
    const pendingForm=ctx.draft.pending_form;
    // If we already have qty+form from a prior "risoles frozen N pack" → add to cart directly
    if (pendingQty && pendingForm) {
      const form=FORM_F.test(t)?'frozen':FORM_G.test(t)?'goreng':pendingForm;
      if (form==='frozen' && pendingQty<6) {
        ctx.replyText=`Maaf Kak, frozen minimal *6pcs* per pack (Rp55.000/pack).\n\nMau frozen 6pcs? Atau ganti ke *goreng* (siap makan) bisa 3pcs/6pcs/12pcs? 🤍`;
        ctx.handled=true; ctx.log('det','frozen-min-qty'); return;
      }
      const name=capitalize(p)+(form==='frozen'?' Frozen':'');
      const price=(form==='frozen'?55000:pendingQty===3?29000:pendingQty===6?55000:pendingQty===12?105000:29000);
      const d=ctx.draft; const existing=Array.isArray(d.items)?d.items:[];
      existing.push({sku:p+'_'+form,name,qty:pendingQty,form,unit_price:price,pack_size:form==='frozen'?6:pendingQty});
      const lines=existing.map((it,i)=>`${i+1}. ${it.name} ${it.qty}pcs ${it.form} — Rp${Number(it.unit_price||0).toLocaleString('id-ID')}`).join('\n');
      const total=existing.reduce((s,it)=>s+(Number(it.unit_price||0)*Number(it.qty||1)/(Number(it.pack_size)||1)),0);
      ctx.updateDraft({items:existing,subtotal:total,state:'ordering',pending_form:null,pending_qty:null,pending_product:null});
      ctx.saveDraft();
      ctx.replyText=`Siap Kak! Berikut pesanannya ya 🤍\n\n${lines}\n\n💰 Total: Rp${Number(total).toLocaleString('id-ID')}\n\nMau tambah yang lain? Atau ketik *cukup* ya Kak 🤍`;
      ctx.handled=true; ctx.log('det','order-from-pending'); return;
    }
    // Otherwise: save pending and ask for qty+form (only when clear order intent or active cart)
    if (ORDER_RE.test(t) || cart.length>0 || state==='ordering') {
      const savedForm=FORM_F.test(t)?'frozen':FORM_G.test(t)?'goreng':null;
      ctx.updateDraft({pending_product:p,...(savedForm?{pending_form:savedForm}:{})}); ctx.saveDraft();
      const formHint=savedForm?`${savedForm} `:'';
      ctx.replyText=`Mau ${p} ${formHint}berapa pcs ya Kak? ${savedForm?'':'Goreng (siap makan) atau frozen (stok rumah)? '}🤍`;
  ...
  if (!PRODUCT_RE.test(t) && QTY_RE.test(t)) {
    ...
    if (pending) {
      const qty=parseInt((t.match(QTY_RE)||[])[1]||'6',10);
      // Use pending_form from draft if no form word in current text
      const form=FORM_F.test(t)?'frozen':FORM_G.test(t)?'goreng':(ctx.draft.pending_form||'goreng');
      // Frozen minimum 6pcs
      if (form==='frozen' && qty>=1 && qty<6) {
        ctx.replyText=`Maaf Kak, frozen minimal *6pcs* per pack (Rp55.000/pack).\n\nMau frozen 6pcs? Atau ganti ke *goreng* (siap makan) bisa 3pcs/6pcs/12pcs? 🤍`;
        ctx.handled=true; ctx.log('det','frozen-min-qty'); return;
      }
      ctx.updateDraft({pending_product:null,pending_form:null,pending_qty:null});
      const name=capitalize(pending)+(form==='frozen'?' Frozen':'');
      const price=(form==='frozen'?55000:qty===3?29000:qty===6?55000:qty===12?105000:29000);
      const d=ctx.draft; const existing=Array.isArray(d.items)?d.items:[];
      existing.push({sku:pending+'_'+form,name,qty,form,unit_price:price,pack_size:form==='frozen'?6:qty});
      const lines=existing.map((it,i)=>`${i+1}. ${it.name} ${it.qty}pcs ${it.form} — Rp${Number(it.unit_price||0).toLocaleString('id-ID')}`).join('\n');
      const total=existing.reduce((s,it)=>s+(Number(it.unit_price||0)*Number(it.qty||1)/(Number(it.pack_size)||1)),0);
      ctx.updateDraft({items:existing,subtotal:total,state:'ordering'});
      ctx.saveDraft();
      ctx.replyText=`Siap Kak! Berikut pesanannya ya 🤍\n\n${lines}\n\nTotal: Rp${Number(total).toLocaleString('id-ID')}\n\nMau tambah yang lain? Atau ketik *cukup* ya Kak 🤍`;
  ...
  if (ORDER_RE.test(t) && !PRODUCT_RE.test(t) && t.length<40) {
    if (FORM_F.test(t) || FORM_G.test(t) || (QTY_RE.test(t) && /risoles\b/i.test(t))) {
      const form=FORM_F.test(t)?'frozen':'goreng';
      const qtyM=t.match(QTY_RE);
      // "1 pack" frozen = 6pcs (1 pack = 6pcs unit)
      let qty=qtyM?parseInt(qtyM[1],10):null;
      if (qty && /pack\b/i.test(t) && form==='frozen') qty=qty*6;
      ctx.updateDraft({pending_form:form,...(qty?{pending_qty:qty}:{})}); ctx.saveDraft();
      const hint=form==='frozen'?'(min 6pcs/pack = Rp55.000)':'(3pcs / 6pcs / 12pcs)';
      ctx.replyText=`Mau risoles *${form}*${qty?' '+qty+'pcs':''} ya Kak! 🤍\nPilih variannya:\n\n• Ayam Sayur\n• Smoked Beef Mayo\n• Ragout Creamy\n• Ayam Mercon Chili Oil\n\n${hint}`;
    } else {
  ...
  if (state==='awaiting_name' && t.length>3 && t.length<200 && !MENU_RE.test(t) && !RESET_RE.test(t)) {
    ...
    if (isPickup) {
      ctx.replyText=`Hai Kak ${name}! Pesanan pickup dicatat ya 🤍\n\nTinggal datang ke Jl Nusa Indah Raya blok O no 10, Cipinang Muara, Jatinegara. Ada yang mau diubah?`;
    } else if (addr) {
      // nama + alamat dalam satu pesan → kirim button sekali, lanjut ke lokasi
      try { require('../wa-sender.cjs').sendLocationRequest(ctx.from,'Share lokasi kamu ya Kak buat akurasi pengiriman 📍'); } catch(_) {}
      ctx.replyText=`Hai Kak ${name}! Alamat sudah dicatat.\n\nSekarang klik tombol *Kirim Lokasi* di atas biar Mintu hitung ongkir & kirim invoice ya Kak.`;
    } else {
      // hanya nama → tanya alamat + kirim button share lokasi sekaligus
      try { require('../wa-sender.cjs').sendLocationRequest(ctx.from,'Share lokasi kamu ya Kak buat akurasi pengiriman 📍'); } catch(_) {}
      ctx.replyText=`Hai Kak ${name}! 🤍\n\nSekarang ketik alamat lengkap pengiriman ya, atau klik tombol *Kirim Lokasi* di atas.`;
    }
  ...
  if (state==='awaiting_address' && t.length>10 && !MENU_RE.test(t) && !RESET_RE.test(t)) {
    ...
    try { require('../wa-sender.cjs').sendLocationRequest(ctx.from,'Share lokasi kamu ya Kak buat akurasi pengiriman'); } catch(_) {}
    ctx.replyText=`Alamat sudah dicatat.\n\nKlik tombol *Kirim Lokasi* di atas biar Mintu hitung ongkir & kirim invoice ya Kak.`;
