...
async function _sendCatalogImage(ctx) {
  if (!_catalogMediaId) {
    try {
      const res=await require('../wa-sender.cjs').uploadMedia('/docker/wa-webhook-sbsr/static/catalog-image.jpeg','image/jpeg');
      _catalogMediaId=res?.id||null;
    } catch(_) { _catalogMediaId=null; }
  }
  if (_catalogMediaId) await require('../wa-sender.cjs').sendImage(ctx.from,_catalogMediaId);
}
