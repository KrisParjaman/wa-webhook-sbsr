...
async function handler(ctx) {
  ...
  if (PRODUCT_RE.test(t) && QTY_RE.test(t)) {
    ...
    for (const tr of triples) {
      ...
      if (tr.form==='frozen' && tr.qty<6) {
        ctx.replyText=`Maaf Kak, frozen minimal *6pcs* (1 pack isi 12pcs). Harga per pack Rp55.000.\n\nMau ganti ke *goreng* (siap makan) bisa 3pcs/6pcs/12pcs? Atau frozen 6pcs? 🤍`;
  ...
  if (!PRODUCT_RE.test(t) && QTY_RE.test(t)) {
    ...
    if (pending) {
      ...
      existing.push({sku:pending+'_'+form,name,qty,form,unit_price:price,pack_size:form==='frozen'?12:qty});
      const lines=existing.map((it,i)=>`${i+1}. ${it.name} ${it.qty}pcs ${it.form} — Rp${Number(it.unit_price||0).toLocaleString('id-ID')}`).join('\n');
