...
async function handler(ctx) {
  ...
  // 9. Payment OK (must run before confirm when state=confirmed)
  const OK_RE = /^(?:ok|oke|ya|iya|bayar|lanjut|gas|siap|deal|okay|y|yes|go|proses)\b/i;
  if (state==='confirmed' && cart.length>0 && OK_RE.test(t) && t.length<10) {
    ctx.replyText='💳 Memproses pembayaran... 🤍';
    ctx.handled=true; ctx.log('det','payment-start');
    try {
      const total=cart.reduce((s,it)=>s+Number(it.unit_price||0),0)+(ctx.draft.ongkir||0);
      const orderKey=[ctx.from,Date.now(),total].join('_');
      ctx.updateDraft({state:'awaiting_proof',grand_total:total,payment_order_key:orderKey,payment_sent_at:new Date().toISOString()});
      ctx.saveDraft();
      const cp=require('child_process');
      const payload=JSON.stringify({phone:'+'+ctx.from.replace(/[^0-9]/g,''),customer_name:ctx.draft.customer_name||'',grand_total:total,order_key:orderKey});
      ctx.log('det','spawning sentuh-payment orderKey='+orderKey);
      cp.execFile('docker',['exec','sbsr-openclaw-1','node','/data/sentuhrasa-pdf/scripts/sentuh-payment.mjs',payload],{timeout:30000},async(err,stdout,stderr)=>{
        ctx.log('det','payment exit='+(err?.code||0)+' out='+(stdout||'').slice(0,80));
        try {
          const lines=String(stdout||'').trim().split('\n');
          const r=JSON.parse(lines[lines.length-1]||'{}');
          if (r.userMessage) await ctx.reply(r.userMessage);
          ctx.log('det','payment-qris='+(r.qrisSent?'ok':'no'));
        } catch(e2) { ctx.log('det','payment-parse-err: '+e2.message); }
      });
    } catch(e) { ctx.log('det','payment-err: '+e.message); }
    return;
  }

  // 10. Confirm / done ordering (NOT when state=confirmed — payment handles that)
  if (state!=='confirmed' && cart.length>0 && (DONE_RE.test(t) || (CONFIRM_RE.test(t) && t.length<8))) {
