...
const _cat = require('../catalog-pg.cjs');
...
// Matches all real variants + generic "ayam/risol/risoles" → resolved via _cat.detectVariant
const PRODUCT_RE = /(ayam\s*sayur\s*pedas|ayam\s*sayur|smoked\s*beef\s*mayo|smoked\s*beef|ragout\s*creamy|ragout|ayam\s*mercon|mercon|chili\s*oil|risoles?\s*ayam|risoles?|ayam|mix)/i;
...
async function handler(ctx) {
  ...
  if (PRODUCT_RE.test(t) && QTY_RE.test(t)) {
    ...
    // Note: PRODUCT_RE.source already includes outer (...), so use it directly — m[1]=variant, m[2]=qty
    const re=new RegExp(PRODUCT_RE.source+'\\s*[^0-9]*(\\d+)\\s*(?:pcs|pack|buah|box|pc)?','gi');
    const triples=[];
    let m;
    while ((m=re.exec(t))!==null) {
      const prod=m[1], qty=parseInt(m[2],10)||6;
    ...
    for (const tr of triples) {
      const variant=_cat.detectVariant(tr.prod)||'ayam_sayur';
      const name=_cat.lookupDisplayName(variant,tr.form);
      const frozenQty=tr.form==='frozen'?6:tr.qty;  // frozen: always 6pcs per pack
      if (tr.form==='frozen' && tr.qty>=1 && tr.qty<6) {
        const frozenPrice=_cat.lookupPrice(variant,'frozen',6)||55000;
        ctx.replyText=`Maaf Kak, frozen minimal *6pcs* per pack (Rp${frozenPrice.toLocaleString('id-ID')}/pack).\n\nMau frozen 6pcs? Atau ganti ke *goreng* (siap makan) bisa 3pcs/6pcs/12pcs? 🤍`;
        ctx.handled=true; ctx.log('det','frozen-min-qty'); return;
      }
      const price=_cat.lookupPrice(variant,tr.form,frozenQty)||55000;
      existing.push({sku:variant+'_'+tr.form+'_'+frozenQty,name,qty:frozenQty,form:tr.form,unit_price:price,pack_size:frozenQty});
    }
  ...
  if (!PRODUCT_RE.test(t) && QTY_RE.test(t)) {
    ...
    if (pending) {
      ...
      const variant=_cat.detectVariant(pending)||'ayam_sayur';
      const name=_cat.lookupDisplayName(variant,form);
      const actualQty=form==='frozen'?6:qty;
      const price=_cat.lookupPrice(variant,form,actualQty)||55000;
      const d=ctx.draft; const existing=Array.isArray(d.items)?d.items:[];
      existing.push({sku:variant+'_'+form+'_'+actualQty,name,qty:actualQty,form,unit_price:price,pack_size:actualQty});
      const lines=existing.map((it,i)=>`${i+1}. ${it.name} ${it.qty}pcs ${it.form} — Rp${Number(it.unit_price||0).toLocaleString('id-ID')}`).join('\n');
