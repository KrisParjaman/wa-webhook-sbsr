...
async function handler(ctx) {
  ...
  // 13. Payment — customer says OK to invoice
  ...
  if (state==='confirmed' && cart.length>0 && OK_RE.test(t) && t.length<10) {
    ...
    try {
      ...
      ctx.saveDraft();
      // Spawn sentuh-payment.mjs via Docker
      ...
      ctx.log('det','spawning sentuh-payment...');
