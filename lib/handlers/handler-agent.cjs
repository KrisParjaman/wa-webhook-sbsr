...
async function handler(ctx) {
  ...
  if (PRODUCT_RE.test(t) && QTY_RE.test(t)) {
    ...
    if (allP.length===1 && allQ.length===1) {
      ...
      // Multiple products — parse each segment and add_to_cart
      ...
      for (const seg of segments) {
        ...
        if (p && q) {
          ...
          try { const r=await require('../agent/tools.cjs').execute('add_to_cart',{variant:p,qty:parseInt(q,10),form:f},ctx); if (r.ok) added++; } catch(_){}
        }
      }
      if (added>0) {
        const cartNow=ctx.cart||[];
        ...
        const total=cartNow.reduce((s,it)=>s+(Number(it.unit_price||0)*Number(it.qty||1)/(Number(it.pack_size)||1)),0);
        ctx.updateDraft({state:'ordering',pending_product:null});
        ctx.replyText=`✅ ${added} item ditambahkan:\n\n${lines}\n\nTotal: Rp${Number(total).toLocaleString('id-ID')}\n\nMau tambah? Atau ketik *cukup* 🤍`;
      } else {
