...
async function handler(ctx) {
  ...
  if (t.startsWith('[location:') && cart.length>0) {
    ...
    if (m) {
      ...
      if (ctx.draft.customer_name && ctx.draft.address_text) {
        // Fire address-and-quote async
        try { require('../address-handler.cjs').tryHandleAddressAndQuote(ctx.from,[ctx.draft.customer_name,ctx.draft.address_text,`https://maps.google.com/?q=${lat},${lng}`].join('\n')).catch(e=>ctx.log('det','quote-err: '+e.message)); } catch(_) {}
        // Send invoice summary immediately while Biteship works in background
        ...
        const ppn=Math.round(total*0.02);
        ctx.replyText=`📍 *Lokasi Diterima!*\n\n📋 *Invoice Sementara:*\n${items}\n\n💰 Subtotal: Rp${Number(total).toLocaleString('id-ID')}\n🧾 PPN 2%: Rp${Number(ppn).toLocaleString('id-ID')}\n\n🔜 *Grand Total + Ongkir* akan dihitung admin & dikirim ke WhatsApp ini ya Kak 🤍\n\n_Estimasi ongkir Gojek/Grab ~Rp15.000-30.000 (Jabodetabek)_`;
  ...
  if (state==='awaiting_name' && t.length>3 && t.length<200 && !MENU_RE.test(t) && !RESET_RE.test(t)) {
    ...
    // Fire address-and-quote for delivery (Biteship + ongkir + invoice)
    if (!isPickup && addr) {
      try {
        const ah=require('../address-handler.cjs');
        const quoteText=[name,addr].join('\n');
        // Fire address-and-quote async
        ah.tryHandleAddressAndQuote(ctx.from, quoteText).catch(e=>ctx.log('det','quote-err: '+e.message));
        // Send invoice immediately
        const _items=cart.map((it,i)=>`${i+1}. ${it.name} ${it.qty}pcs ${it.form} — Rp${Number(it.unit_price||0).toLocaleString('id-ID')}`).join('\n');
        const _total=cart.reduce((s,it)=>s+Number(it.unit_price||0),0);
        ctx.replyText=`Hai Kak ${name}! 🤍\n\n📋 *Invoice Sementara:*\n${_items}\n\n💰 Subtotal: Rp${Number(_total).toLocaleString('id-ID')}\n🧾 PPN 2%: Rp${Number(Math.round(_total*0.02)).toLocaleString('id-ID')}\n\n🔜 *Grand Total + Ongkir* menyusul ya Kak 🤍\n\n_Silahkan share lokasi via WhatsApp untuk akurasi ongkir_`;
      } catch(_) { ctx.replyText=`Hai Kak ${name}! Alamat dicatat ya 🤍\n\nPesanan akan diproses admin. Ada yang mau diubah?`; }
    } else {
  ...
  if (state==='awaiting_address' && t.length>10 && !MENU_RE.test(t) && !RESET_RE.test(t)) {
    ...
    try { require('../wa-sender.cjs').sendLocationRequest(ctx.from,'Share lokasi kamu ya Kak buat akurasi pengiriman 📍'); } catch(_) {}
    const _items2=cart.map((it,i)=>`${i+1}. ${it.name} ${it.qty}pcs ${it.form} — Rp${Number(it.unit_price||0).toLocaleString('id-ID')}`).join('\n');
    const _total2=cart.reduce((s,it)=>s+Number(it.unit_price||0),0);
    ctx.replyText=`Alamat dicatat! 🤍\n\n📋 *Invoice Sementara:*\n${_items2}\n\n💰 Subtotal: Rp${Number(_total2).toLocaleString('id-ID')}\n🧾 PPN 2%: Rp${Number(Math.round(_total2*0.02)).toLocaleString('id-ID')}\n\n🔜 *Grand Total + Ongkir* menyusul ya Kak 🤍\n\n_Silahkan share lokasi untuk akurasi ongkir_`;
