...
async function handler(ctx) {
  ...
  if (PRODUCT_RE.test(t) && QTY_RE.test(t)) {
    ...
    if (allP.length===1 && allQ.length===1) {
      try {
        const r=await require('../agent/tools.cjs').execute('add_to_cart',{variant,form,qty},ctx);
        if (r.ok) {
          ...
          ctx.updateDraft({state:'ordering',pending_product:null});
          ctx.replyText=`✅ Ditambahkan:\n\n${lines}\n\nTotal: Rp${Number(total).toLocaleString('id-ID')}\n\nMau tambah? Atau ketik *cukup* 🤍`;
          ctx.handled=true; ctx.log('det','order'); return;
        }
      } catch(_){}
    } else {
      ctx.replyText=`Mintu catat: ${allP.join(', ')}. ${FORM_F.test(t)?'Frozen':FORM_G.test(t)?'Goreng':'Goreng atau frozen'}? Berapa pcs masing-masing? 🤍`;
  ...
  if (ORDER_RE.test(t) && !PRODUCT_RE.test(t) && t.length<30) {
    ctx.replyText='Siap Kak! Mau varian apa? Contoh: "ayam sayur 6pcs goreng" 🤍\nAtau mau lihat *menu* dulu?';
  ...
  // 13. DeepSeek fallback
  if (t.length<5) { ctx.log('det','short'); return; }
  ctx.log('det','llm');
  try {
    const r=await callDeepSeek(ctx);
    if (r) { ctx.replyText=r; ctx.handled=true; }
  } catch(_){}
}
