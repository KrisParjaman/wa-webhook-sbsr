...
async function handler(ctx) {
  ...
  if (PRODUCT_RE.test(t) && ORDER_INTENT_RE.test(t) && !QTY_RE.test(t)) {
    ...
    ctx.updateDraft({ pending_product: p });
    ctx.saveDraft();
  ...
  // Qty + form mentioned but NO product → use pending_product from draft
  if (!PRODUCT_RE.test(t) && QTY_RE.test(t) && (FORM_GORENG_RE.test(t) || FORM_FROZEN_RE.test(t))) {
    ...
    if (pending) {
      ...
      try {
        const tools = require('../agent/tools.cjs');
        const r = await tools.execute('add_to_cart', { variant: pending, form, qty }, ctx);
        if (r.ok) {
          const item = r.result.item||{};
          ctx.updateDraft({ state: 'ordering' });
          ctx.saveDraft();
          ctx.replyText = `Siap! ${item.name||pending} ${qty}pcs ${form} dicatat ya 🤍. Mau tambah apa lagi? Atau ketik *cukup* kalau udah.`;
          ctx.handled = true; ctx.log('det','order-pending'); return;
        }
      } catch(_){}
    }
  }
...
${cartStr === 'kosong' ? 'Customer belum order. Kalau dia mau pesan, tanya varian + qty + goreng/frozen.' : 'Customer SUDAH order item di atas. JANGAN suruh dia pesan lagi. Tanyakan: mau tambah? atau cukup? Kalau dia bilang cukup/udah/itu aja, arahkan ke nama & alamat.'}`;
  ...
  const body = JSON.stringify({
    ...
    messages: [
      { role: 'system', content: 'Kamu Mintu, CS Sentuh Rasa. Produk: Ayam Sayur, Smoked Beef Mayo, Ragout Creamy, Ayam Mercon Chili Oil, Ayam Sayur Pedas, Mix Risol. Hanya 2 form: Goreng (siap makan) atau Frozen (stok rumah). Add-on: Chili Sauce, Thermal Bag, Ice Gel, Mika Bag, Greeting Card, Iced Tea, Matcha. Jawab NATURAL seperti ngobrol WA, 1-2 kalimat, Bahasa Indonesia santai. Akhiri pakai 🤍.' },
  ...
  // Guard: reject hallucinated products
