...
async function handler(ctx) {
  ...
  if (PRODUCT_RE.test(t) && QTY_RE.test(t)) {
    ...
    if (allP.length===1 && allQ.length===1) {
      ...
      // Multiple products — parse each segment
      ...
      for (const seg of segments) {
        ...
        if (p && q) {
          ...
          missing.push(capitalize(p)); // product mentioned but no qty
        }
      }
      // Save cart + reply
      const d=ctx.draft;
      ...
      if (merged.length>0) {
        ...
        ctx.log('det',`cart=${merged.length} total=${total}`);
        let suffix='';
        if (missing.length>0) suffix=`\n\n⚠️ ${missing.join(', ')} — belum ada jumlah pcs-nya. Mau berapa pcs ya Kak?`;
        ctx.replyText=`Siap Kak! Berikut keranjangnya:\n\n${lines}\n\n💰 Total: Rp${Number(total).toLocaleString('id-ID')}${suffix}\n\nMau tambah yang lain? Atau ketik *cukup* ya Kak 🤍`;
      } else if (missing.length>0) {
        ctx.replyText=`Mintu catat: ${missing.join(', ')}. Mau berapa pcs? Dan yang goreng (siap makan) atau frozen (stok rumah)? 🤍`;
      } else {
  ...
  if (cart.length>0 && (DONE_RE.test(t) || (CONFIRM_RE.test(t) && t.length<8))) {
    ...
    ctx.replyText=`📋 *Pesanan Dikonfirmasi*\n\n${lines}\n\n💰 Total: Rp${Number(total).toLocaleString('id-ID')}\n\nSelanjutnya: *nama penerima* & *alamat lengkap* ya Kak 🤍`;
  ...
  if (state==='awaiting_name' && t.length>3 && t.length<200 && !MENU_RE.test(t) && !RESET_RE.test(t)) {
    ...
    if (addr) {
      ...
      patch.state='awaiting_delivery_method';
  ...
  if (state==='awaiting_address' && t.length>10 && !MENU_RE.test(t) && !RESET_RE.test(t)) {
    ...
    // Send location request button for precise delivery
    try {
      const wa = require('../wa-sender.cjs');
      wa.sendLocationRequest(ctx.from, 'Share lokasi kamu ya Kak buat akurasi pengiriman 📍');
    } catch(_) {}
    ctx.replyText='Alamat dicatat! 🤍\n\nBiar lebih akurat, klik tombol *Kirim Lokasi* di atas ya Kak~\n\nMau *Delivery* (Gojek/Grab/Paxel) atau *Pickup* (ambil di Cipinang)?';
