...
async function handler(ctx) {
  ...
  if (t.startsWith('[location:') && cart.length>0) {
    ...
    if (m) {
      ...
      if (ctx.draft.customer_name && ctx.draft.address_text) {
        // Call Biteship for real ongkir
        ctx.replyText='📍 Lokasi diterima! Mintu sedang hitung ongkir real-time... 🤍';
        ctx.handled=true;
        try {
  ...
  if (state==='awaiting_name' && t.length>3 && t.length<200 && !MENU_RE.test(t) && !RESET_RE.test(t)) {
    ...
    if (!isPickup && addr) {
      ...
      try {
        const ah=require('../address-handler.cjs');
        ah.tryHandleAddressAndQuote(ctx.from,[name,addr].join('\n')).catch(e=>ctx.log('det','quote-err: '+e.message));
      } catch(_) {}
      ctx.replyText=`Hai Kak ${name}! 🤍\n\n📋 *INVOICE*\n${_items}\n\n💰 Subtotal: Rp${Number(_total).toLocaleString('id-ID')}\n🚀 Ongkir (Gojek/Grab est.): Rp20.000\n🧾 PPN 2%: Rp${Number(Math.round(_total*0.02)).toLocaleString('id-ID')}\n\n💳 *TOTAL: Rp${Number(_total+20000+Math.round(_total*0.02)).toLocaleString('id-ID')}*\n\nPembayaran via:\n• QRIS / GoPay / OVO\n• Transfer BCA 1234567890 a.n. Sentuh Rasa\n\n_Silahkan share lokasi untuk ongkir akurat & kirim QRIS ya Kak 🤍_`;
    } else {
  ...
  if (state==='awaiting_address' && t.length>10 && !MENU_RE.test(t) && !RESET_RE.test(t)) {
    ...
    const _ppn2=Math.round(_total2*0.02);
    ctx.replyText=`Alamat dicatat! 🤍\n\n📋 *INVOICE*\n${_items2}\n\n💰 Subtotal: Rp${Number(_total2).toLocaleString('id-ID')}\n🚀 Ongkir (Gojek/Grab est.): Rp20.000\n🧾 PPN 2%: Rp${Number(_ppn2).toLocaleString('id-ID')}\n\n💳 *TOTAL: Rp${Number(_total2+20000+_ppn2).toLocaleString('id-ID')}*\n\nPembayaran via:\n• QRIS / GoPay / OVO\n• Transfer BCA 1234567890 a.n. Sentuh Rasa\n\n_Silahkan share lokasi & bukti bayar ya Kak 🤍_`;
