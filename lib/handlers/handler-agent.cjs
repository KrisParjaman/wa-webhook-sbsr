...
async function handler(ctx) {
  ...
  // ── 8. DeepSeek fallback (complex/unmatched messages) ──────────
...
async function callDeepSeek(ctx) {
  ...
  const prompt = `Kamu Mintu, CS Sentuh Rasa — Risol Otentik. Jawab 1-2 kalimat natural & warm.

Order saat ini: [${cartStr}]
Customer: ${ctx.text}

${cartStr === 'kosong' ? 'Kalau customer mau pesan, tanya varian + qty + goreng/frozen.' : 'Konfirmasi pesanan, tanyakan ada tambahan.'}`;

  const body = JSON.stringify({
    model: 'deepseek/deepseek-chat',
    messages: [
      { role: 'system', content: 'Kamu Mintu, CS ramah Sentuh Rasa. Jawab singkat, natural, Bahasa Indonesia. Akhiri 🤍.' },
