...
function match(state, ctx) {
  ...
  // Only match cancel/ga-jadi if NOT followed by a specific product name
  if (CANCEL_RE.test(t)) {
    const afterCancel=t.match(/(?:ga\s*jadi|gak\s*jadi|nggak\s*jadi|batal(?:in)?|cancel|hapus)\s+(.+)/i);
    if (afterCancel && /\b(?:ayam|smoked|ragout|mercon|chili|pedas|original|creamy|mix|risol|frozen|goreng|saus|thermal|ice|mika|greeting)\b/i.test(afterCancel[1])) return false;
    return true;
  }
...
async function handler(ctx) {
  ...
  // ── Simple cancel/restart in non-checkout state ──────────────────
  if ((CANCEL_RE.test(t) || RESTART_RE.test(t)) && (!state || state === 'initial' || state === 'main_menu')) {
    ctx.updateDraft({ state: 'initial', items: [], addons: [], customer_name: '', address_text: '', destination: null, gmaps_link: '', delivery_mode: '', use_case: '' });
    ctx.saveDraft();
    ctx.replyText = 'Siap Kak! Mau lihat menu atau langsung order? Ketik aja~ 🤍';
    ctx.handled = true;
    return;
  }
