...
let _sendReaction, _sendTyping, _sendSbsrDeliveryBtns, _sendSbsrAddonOffer;
function init(o){
  ...
  _AP=o.ADMIN_PHONES||'';
  _sendReaction=o.sendReaction||function(){};
  _sendTyping=o.sendTypingIndicator||function(){};
  _sendSbsrDeliveryBtns=o.sendSbsrDeliveryMethodButtons||function(){};
  _sendSbsrAddonOffer=o.sendSbsrAddonOffer||function(){};
}

// ── Stubs for server.js globals not yet injected ────────────────────
const sendWhatsAppCatalog_stub = () => Promise.resolve();
const sendCatalogDeterministicFallback_stub = () => Promise.resolve();
const resolveGmapsUrlBridge_stub = () => Promise.resolve(null);
const geocodeAddressTextBridge_stub = () => Promise.resolve(null);
const decodeMapsPlaceFromUrlBridge_stub = () => null;
const parseDirectGmapsCoordsBridge_stub = () => null;
const extractCoordsFromMapsUrlBridge_stub = () => null;
const geocodeTypedAddressWithFallback_stub = () => Promise.resolve(null);
const reverseGeocodeCoordsBridge_stub = () => Promise.resolve(null);
const resolveLocationDisplayBridge_stub = () => Promise.resolve({place_address:'',place_label:'',source:'stub'});
const finalizeSbsrCoords_stub = (a,b) => ({lat:a,lng:b});
const isSbsrCoordInRegion_stub = () => false;
const haversineKm_stub = () => 0;
const normalizePhone08_stub = v => String(v||'').replace(/[^0-9]/g,'');
const pickNonEmpty_stub = (...v) => {for(const x of v){const s=String(x||'').trim();if(s)return s}return''};
const fmtRupiah_stub = n => 'Rp '+(Number(n)||0).toLocaleString('id-ID');
const parseRupiah_stub = s => {const m=String(s||'').match(/[\d.]+/);return m?parseInt(m[0].replace(/\./g,''),10):0};
const looksLikeAddress_stub = () => false;
const isSyntheticMsg_stub = () => false;
const extractCustomerName_stub = () => null;
const findNameInChatHistory_stub = () => null;
const isNameTokens_stub = () => false;
const sendWhatsAppCatalog = () => Promise.resolve();
const sendCatalogDeterministicFallback = () => Promise.resolve();
const tryHandleAddressAndQuote = () => Promise.resolve(false);
const sbsrRouterStateLabel = (d) => (d&&d.state)||'initial';
const sbsrRouterLogState = () => {};
const sbsrRouterLogRail = () => {};
const sbsrRouterLogSkipped = () => {};
const sbsrRetrieveMemoryContext = () => Promise.resolve('');
const sbsrMemoryEnabled = () => false;
const SBSR_OUT_OF_CONTEXT_STATES = new Set();
const formatCatalogForLLM = () => '';
const formatFaqForLLM = () => '';
const loadSbsrOrders = () => ({});
const saveSbsrOrders = () => {};
const buildItemsFromPending = () => ({items:[],subtotal:0});
const sendSbsrDeliveryMethodButtons = () => {};
const sendSbsrAddonOffer = () => {};
const sendSbsrUseCasePrompt = () => {};
const sendWelcomeWithCatalog = () => {};
const getSbsrFinancePhones = () => [];
const isCheckoutActiveState = () => false;
const isRestartIntent = () => false;
const isCancelIntent = () => false;
const isMenuIntent = () => false;
const isManualResetIntent = () => false;
const shouldResetSbsrSessionOnReentry = () => false;
const isAddMoreIntent = () => false;
const isOrderLikeText = () => false;
const clearSbsrCheckoutForCancel = () => false;
const sbsrDraftHasDestination = () => false;
const isSbsrCheckoutCollectionActive = () => false;
let sbsrLlmClassifierEnabled = true;
const CLASSIFIER_SKIP_RE = /(?!)/;
const CLASSIFIER_MAPS_SKIP_RE = /(?!)/;
const CLASSIFIER_MAX_CLARIFY = 3;
const PROCESSED_MSG_IDS = new Map();
const PROCESSED_MSG_MAX = 500;
const getSbsrDeterministicMissingStateMessage = () => '';
const sendSbsrLocationPromptMessage = () => {};
const getSbsrPickupDetailMessage = () => '';
const sniffMapsLinkFromCustomer = () => false;
const looksLikeAddress = () => false;
const isSyntheticMsg = () => false;
const extractCustomerName = () => null;
const findNameInChatHistory = () => null;
const isNameTokens = () => false;
const getClarifyCount = () => 0;
const resetClarifyCount = () => {};
const incrementClarifyCount = () => 0;
const buildSbsrCartSummary = () => '(empty)';
const inferCatalogProductMode = () => null;
const looksLikeAddressPinMismatch = () => false;
const SBSR_PRODUCT_SELECTION_INTENT_RE = /(?!)/;

const tryHandleBuktiOcrFailedManualReview = () => Promise.resolve(false);
const tryHandleBuktiAuto = () => Promise.resolve(false);
const tryHandleWhatsAppLocation = () => Promise.resolve(false);
const tryHandleInvoiceOk = () => Promise.resolve(false);
const tryHandleOrderConfirm = () => Promise.resolve(false);
const tryHandleMeetingPackageConfirm = () => Promise.resolve(false);
const tryHandlePinConfirm = () => Promise.resolve(false);
const tryHandleNameCapture = () => Promise.resolve(false);
const tryHandleAddressTextCapture = () => Promise.resolve(false);
const tryHandleFaq = () => Promise.resolve(false);
const tryHandleFreeTextOrder = () => Promise.resolve(false);
const tryHandlePickupFlow = () => Promise.resolve(false);
const tryHandleFrozenCourierChoice = () => Promise.resolve(false);
const tryHandleDeliveryMethodSelection = () => Promise.resolve(false);
const tryHandleUseCaseRouter = () => Promise.resolve(false);
const tryHandleAddressPinConfirm = () => Promise.resolve(false);
const tryHandleDeterministicGreeting = () => Promise.resolve(false);
const tryHandleBareMapsUrl = () => Promise.resolve(false);
const tryHandleGlobalAddMore = () => Promise.resolve(false);
const tryHandleMissingFormInquiry = () => Promise.resolve(false);
const tryHandleMissingFormClarification = () => Promise.resolve(false);
const tryHandleTextVariantSelection = () => Promise.resolve(false);
const tryHandleOngkirCheck = () => Promise.resolve(false);
const tryHandleDestinationCheck = () => Promise.resolve(false);
const tryHandleCourierOverride = () => Promise.resolve(false);
const tryHandleUrlEcho = () => Promise.resolve(false);
const tryHandleAdminHandoff = () => Promise.resolve(false);
const tryHandleKitchenReady = () => Promise.resolve(false);
const tryHandleMainMenuQuestionChoice = () => Promise.resolve(false);
const tryHandleCatalogRequest = () => Promise.resolve(false);
const tryHandleDeliveryConfirm = () => Promise.resolve(false);
const tryHandleAmbiguousConfirm = () => Promise.resolve(false);
const tryHandleIgApproval = () => Promise.resolve(false);
const tryHandlePOApproval = () => Promise.resolve(false);
const tryHandlePOCreate = () => Promise.resolve(false);
const tryHandleSaldo = () => Promise.resolve(false);
const tryHandleIgTopicReply = () => Promise.resolve(false);
const tryHandleIgPost = () => Promise.resolve(false);
const tryHandleOutOfContextHandoff = () => Promise.resolve(false);
const tryHandleOocDuringCheckout = () => Promise.resolve(false);
const tryHandleAdminCmd = () => Promise.resolve(false);
const tryHandlePaymentReviewStatusIntent = () => Promise.resolve(false);
...
async function _processMessage(msg, from, messageId, contactName) {
    ...
    if (msg.type === "text") {
      ...
      if (_sec) {
        const _s = _sec.sanitizeUserText(_raw);
        if (_s.flags.length) {
          try {
            fs.appendFileSync(SECURITY_FLAGS_FILE,
              JSON.stringify({ ts: new Date().toISOString(), from, flags: _s.flags, blocked: _s.blocked, sample: String(_raw).slice(0, 200) }) + "\n");
          } catch (e) { console.error("[security] flag-log err:", e.message); }
          _L("security", from + " " + _sec.summarizeFlags(_s));
        }
        if (_s.blocked) {
        ...
        try {
          if (!ocr) {
            if (await tryHandleBuktiOcrFailedManualReview(from, imgResult.url)) {
              ...
              _sendReaction(from, messageId, "").catch(() => {});
              return;
            }
          }
      ...
      // Route to pipeline v2
      try { _initEngine(); } catch (_) {}
      try {
        var _v2ctxLoc = _ec ? _ec.createContext({
          from: from, messageId: messageId, contactName: contactName,
          rawText: userText, msgType: msg.type, rawMsg: msg,
        }) : null;
        if (_v2ctxLoc && _ep) { await _ep.runPipeline(_v2ctxLoc); }
        if (_v2ctxLoc && _v2ctxLoc.handled) { _sendReaction(from, messageId, "").catch(function(){}); return; }
      } catch (_eLoc) { _L("pipeline", "location err: " + (_eLoc&&_eLoc.message||"?")); }
    }
      ...
      try {
        ...
        if (existing.use_case) {
          ...
          if (existingUseCase === "stock_frozen" && !hasFrozenInOrder) {
            ...
            await _sendSbsrAddonOffer(from, latestDraft);
          ...
          if (inferredMode) {
            ...
            if (_prevState === "awaiting_usecase") {
              ...
              await _sendSbsrAddonOffer(from, _nextDraft);
      ...
      if (msg.interactive?.type === "list_reply") {
        ...
        if (finBtn) {
          ...
          if (btnId.startsWith('sbsr_')) { userText = btnId.replace('sbsr_',''); } else { userText = btnTitle; }
        }
    ...
    // Route SBSR interactive button replies through pipeline
    if (msg.type === "interactive" && msg.interactive?.type === "button_reply") {
      const _btnId = msg.interactive.button_reply.id || "";
      if (_btnId.startsWith("sbsr_") && !_btnId.match(/^sbsr_(approve|reject)_/)) {
        try { _initEngine(); } catch (_) {}
        try {
          var _v2ctxInt = _ec ? _ec.createContext({
            from: from, messageId: messageId, contactName: contactName,
            rawText: userText, msgType: msg.type, rawMsg: msg,
          }) : null;
          if (_v2ctxInt && _ep) { await _ep.runPipeline(_v2ctxInt); }
          _sendReaction(from, messageId, "").catch(function(){});
          return;
        } catch (_e3) { _L("pipeline", "interactive err: " + (_e3&&_e3.message||"?")); }
      }
    }

    _L("msg", contactName + " (" + from + "): " + userText);
    try { _adm.logIncoming(from, userText || ("[" + msg.type + "]"), contactName); } catch (e) { console.error("[admin] log err:", e.message); }
    ...
    _sendTyping(from, messageId).catch(() => {});
    ...
    if (msg.type === "text") {
      ...
      const _activeCheckoutForMenu = (()=>false)(_preDraftForMenu);
    ...
    if (msg.type === "interactive" && msg.interactive && msg.interactive.button_reply) {
      ...
      if (_btnId === "ya_lanjut") {
        ...
        if (_bItems) {
          ...
          await _sendSbsrDeliveryBtns(from);
        ...
        if (_pendingItems.length > 0 || _summary) {
          ...
          if (_pendingItems.length > 0) {
            _built = (function(){return{items:[],subtotal:0}})(_pendingItems, _form);
    ...
    try { (()=>false)(from, userText); } catch (e) { _L("sbsr-maps-sniff", "err: " + e.message); }
    try {
      ...
      if (_acceptLanjut && _needsDelivery && (_hasItems || _hasPendingItems)) {
        ...
        if (!_hasItems && _hasPendingItems) {
          ...
          const _built = (function(){return{items:[],subtotal:0}})(_routerDraft.pending_items, _form);
      ...
      if ((()=>false)(_routerState) && _trimText.length >= 2) {
  ...
  return nudges[state] || "Silakan lanjutkan proses pemesanan ya Kak \u{1f90d}";
    ...
    if (_checkoutLockStates.has(_postState)) {
      ...
      try {
        ...
        var _stateNudge = (()=>"")(from, _LD(from) || {}) || "lanjut ke proses pemesanan ya Kak \u{1f90d}";
        var _oocGuard = [
      ...
      const _fallback = (()=>"")(from, _postDraft);
    ...
    if (!aiReply || !aiReply.trim()) { _L("warn", "Empty response from OpenClaw, skipping WA send"); _sendReaction(from, messageId, "").catch(() => {}); return; }
